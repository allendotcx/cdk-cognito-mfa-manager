...
const getRandomString = () => {
    ...
    const binaryStringItems = randomItems.map(dec =>
...
const hashToBase64url = arrayBuffer => {
  ...
  const stringifiedArrayHash =
    items.reduce((acc, i) => `${acc}${String.fromCharCode(i)}`, '');
  const decodedHash = btoa(stringifiedArrayHash);
  const base64URL =
    decodedHash.replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/, '');
  return base64URL
}
...
async function main() {
  ...
  if (code == null) { // Create random "state"
    ...
    location.href = "https://" + domain + ".auth." + region +
      ".amazoncognito.com/oauth2/authorize?response_type=code&state=" + state +
      "&client_id=" + appClientId + "&redirect_uri=" + redirectURI +
    ...
    if (sessionStorage.getItem("pkce_state") != state) {
      ...
      await fetch("https://" + domain + ".auth." + region +
        ...
        appClientId + "&code_verifier=" + code_verifier + "&redirect_uri=" +
